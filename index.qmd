...
def trim_html_table(html_string, start=None, end=2):
...
mani, targets = fastEDM.create_manifold_and_targets(list(t), list(x), E=E, tau=tau, p=p)

mani = mani[:, ::-1]  # reverse the order of the columns
targets = targets.round(2)

mani_df = pd.DataFrame(mani)
...
```{python}
#| echo: false
point = mani_df.iloc[-1, :].values
target = targets_df.iloc[-1, :].values 
...
```{python}
#| echo: false
for i in range(mani.shape[0]-1): 
  point_i = mani_df.iloc[i, :].values
  target_i = targets_df.iloc[i, 0] 
  plt.plot(point_i, 'o-')
...
# Calculate the distances to the poi nt
distances = np.linalg.norm(mani - point, axis=1)
distances = distances.round(2)
distances_df = pd.DataFrame({"dist": distances})
# distances_df
``` 
...
mani_sorted = mani_df.iloc[order]
targets_sorted = targets_df.iloc[order]
distances_sorted = distances_df.iloc[order]
```
...
# Sort the manifold, targets and distances by distance 
k = len(point) + 1
mani_sorted = mani_sorted.head(k)
targets_sorted = targets_sorted.head(k)
distances_sorted = distances_sorted.head(k)
```
...
```{python}
#| echo: false
for i in range(mani_sorted.shape[0]): 
  point_i = mani_sorted.iloc[i, :].values
  target_i = targets_sorted.iloc[i, 0] 
  plt.plot(point_i, 'o-')
...
#| echo: false
theta = 1.0
weights = np.exp(-theta * distances_sorted / distances_sorted.iloc[0, 0])
...
for i in range(mani_sorted.shape[0]): 
  point_i = mani_sorted.iloc[i, :].values
  target_i = targets_sorted.iloc[i, 0] 
  plt.plot(point_i, 'o-', alpha=weights.iloc[i, 0])
  ...
  plt.plot(xs, ys, '--', color=prev_col, alpha=0.75 * weights.iloc[i, 0])
