...
M, y = fastEDM.create_manifold_and_targets(t[:T], x[:T], E=E, tau=20, p=10)

M_pred, y_pred = fastEDM.create_manifold_and_targets(t[T:], x[T:], E=E, tau=20, p=10)
point = M_pred[-1,:].copy()
...
#| echo: false
distances = np.linalg.norm(manifold - point, axis=1)
distances = pd.DataFrame({"\(d(\mathbf{x}_i, \mathbf{x}^*)\)": distances}) ############ NORMALISED DISTANCES
distances.columns.name = "\(i\)"
...
model = ols.fit(X, Y) #, sample_weight=weights.to_numpy().flatten())
```
...
for i, ax in enumerate(axes):
  ...
  for j, x_j in enumerate(xs):
    ...
    avg_point[i] = x_j
    preds[j] = model.predict(avg_point[np.newaxis,:])
...
for ax in axes:
    ax.plot(X[:,0], X[:,1], Y, zorder=15, linestyle='none', marker='o', alpha=0.1)
    ax.scatter(xx_pred.flatten(), yy_pred.flatten(), predicted, facecolor=(0,0,0,0), s=20, edgecolor=colors[1])
...
## Weighted linear regression plane
...
for ax in axes:
    ax.plot(X[:,0], X[:,1], Y, color='k', zorder=15, linestyle='none', marker='o', alpha=0.1) # alpha must be numeric or None, not <class 'numpy.ndarray'> #################################################
    ax.scatter(xx_pred.flatten(), yy_pred.flatten(), predicted, s=20, facecolor=(0,0,0,0), edgecolor=colors[1])
    ax.set_xlabel('Price', fontsize=12)
    ax.set_ylabel('Advertising', fontsize=12)
    ax.set_zlabel('Pie Sales', fontsize=12)
    ax.locator_params(nbins=4, axis='x')
...
```{python}
def sample_geometric_exponential_sums_fast(T, p, μ):
  result = []
...
rnd.seed(1)
%time rvs = sample_geometric_exponential_sums_fast(1_000, 0.5, 20.0)
```
...
t2 = %timeit -n1 -r1 -q -o sample_geometric_exponential_sums_fast(1_000, 0.5, 20.0)
